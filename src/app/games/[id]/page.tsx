...
const TableView: React.FC<{
  ...
  onUpdateCellValue: (cellValueId: number | null, rowId: number, columnId: number, value: string) => void;
  onAddColumn: (tableId: number, name: string, type: string) => void;
}> = ({ table, columns, rows, cellValues, onAddRow, onUpdateCellValue, onAddColumn }) => {
  ...
  const getCellValue = (rowId: number, columnId: number): {id: number | null, value: string} => {
    const cell = cellValues.find(cv => cv.rowId === rowId && cv.columnId === columnId);
  ...
  const handleCellBlur = () => {
    if (editingCell) {
      ...
      onUpdateCellValue(cellData.id, editingCell.rowId, editingCell.columnId, editValue);
      setEditingCell(null);
    }
  };
                      ...
                      >
                        {isEditing ? (
                          ...
                          <div className="min-h-6 cursor-pointer hover:bg-slate-700/50 px-1 py-0.5 rounded">
                            {cellData.value || <span className="text-slate-600 italic">Click to edit</span>}
                          </div>
                        )}
                      </td>
                    );
...
// Main Game Event Page component
const GameEventPage: React.FC = () => {
  // In a real app, you would get the gameId from the URL params
  const gameId = 1; // This would come from your route, e.g. /game/1
  
  const [tables, setTables] = useState<Table[]>([
    { id: 1, name: 'Characters', gameId },
    { id: 2, name: 'Items', gameId },
    { id: 3, name: 'Locations', gameId }
  ]);
  
  const [columns, setColumns] = useState<Column[]>([
    { id: 1, tableId: 1, name: 'Name', type: 'text', order: 0 },
    { id: 2, tableId: 1, name: 'Class', type: 'text', order: 1 },
    { id: 3, tableId: 1, name: 'Level', type: 'number', order: 2 },
    { id: 4, tableId: 2, name: 'Item Name', type: 'text', order: 0 },
    { id: 5, tableId: 2, name: 'Value', type: 'number', order: 1 },
    { id: 6, tableId: 3, name: 'Location', type: 'text', order: 0 },
    { id: 7, tableId: 3, name: 'Description', type: 'text', order: 1 }
  ]);
  ...
  const [cellValues, setCellValues] = useState<CellValue[]>([
    { id: 1, rowId: 1, columnId: 1, value: 'Gandalf' },
    { id: 2, rowId: 1, columnId: 2, value: 'Wizard' },
    { id: 3, rowId: 1, columnId: 3, value: '20' },
    { id: 4, rowId: 2, columnId: 1, value: 'Aragorn' },
    { id: 5, rowId: 2, columnId: 2, value: 'Ranger' },
    { id: 6, rowId: 2, columnId: 3, value: '15' },
    { id: 7, rowId: 3, columnId: 4, value: 'Magic Sword' },
    { id: 8, rowId: 3, columnId: 5, value: '500' }
  ]);
  ...
  const handleUpdateCellValue = (cellValueId: number | null, rowId: number, columnId: number, value: string) => {
    // If cell value exists, update it
    if (cellValueId) {
      setCellValues(cellValues.map(cv => 
        cv.id === cellValueId ? { ...cv, value } : cv
      ));
    } else {
  ...
  const activeTableColumns = columns.filter(col => col.tableId === activeTableId);
  const activeTableRows = rows.filter(row => row.tableId === activeTableId);
  const relevantCellValues = cellValues.filter(cv => 
