...
export const CreateTableForm: React.FC<{
  gameId: number;
  onCancel: () => void;
  onSubmit: (table: Omit<Table, 'id'>, columns: Omit<Column, 'id' | 'tableId'>[]) => void;
}> = ({ gameId, onCancel, onSubmit }) => {
  const [tableName, setTableName] = useState<string>('');
  const [columns, setColumns] = useState<Array<{ name: string; type: string; order: number }>>([
